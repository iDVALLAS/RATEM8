...
- **NMLS #** displayed in footer (pulled from `ANCHOR_LO.nmls` in `lib/licensing.ts`; each sponsoring entity also disclosed)
...
- **Disclaimer at bottom of homepage:** generated by `buildDisclaimer()` in `lib/licensing.ts`. Sources: anchor LO record (name, NMLS, operating entity) + sponsoring entities per state. Compliance attorney must review the generated paragraph before publishing.
